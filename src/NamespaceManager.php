...
use SemanticGenealogy\ExtraneousLanguage\ExtraneousLanguage;
...
class NamespaceManager {
	...
	public function init() {
		...
		if ( empty( $this->globalVars[static::PREFIX.'wgContLang'] ) ) {
			$this->globalVars[static::PREFIX.'wgContLang'] = $this->extraneousLanguage->fetchByLanguageCode( $this->globalVars['wgLanguageCode'] );
		}
	...
	public static function buildNamespaceIndex( $offset ) {
		...
		$namespaceIndex = array(
			'NS_SGENEALOGY'               => $offset + 0,
			'NS_SGENEALOGY_TALK'          => $offset + 1,
			'NS_SGENEALOGY_TEMPLATE'      => $offset + 2,
			'NS_SGENEALOGY_TEMPLATE_TALK' => $offset + 3,
			'NS_SGENEALOGY_FORM'          => $offset + 4,
			'NS_SGENEALOGY_FORM_TALK'     => $offset + 5,
		);
	...
	public static function initCustomNamespace( &$globalVars ) {
		...
		if ( !isset( $globalVars[static::PREFIX.'wgNamespaceIndex'] ) ) {
			$globalVars[static::PREFIX.'wgNamespaceIndex'] = 2700;
		}
		...
		$defaultSettings = array(
			...
			static::PREFIX.'wgNamespacesWithSemanticLinks',
			static::PREFIX.'wgNamespaceIndex',
		...
		foreach ( $instance->buildNamespaceIndex( $globalVars[static::PREFIX.'wgNamespaceIndex'] ) as $ns => $index ) {
			if ( !$instance->isDefinedConstant( $ns ) ) {
				define( $ns, $index );
			};
		}

		//echo $globalVars['wgLanguageCode'];
	...
	protected function addNamespaceSettings() {
		...
		$smwNamespacesSettings = array(
			NS_SGENEALOGY => true,
			NS_SGENEALOGY_TALK => true,
			NS_SGENEALOGY_TEMPLATE => true,
			NS_SGENEALOGY_TEMPLATE_TALK => true,
			NS_SGENEALOGY_FORM => true,
			NS_SGENEALOGY_FORM_TALK => true,
		);

		// Combine default values with values specified in other places
		// (LocalSettings etc.)
		$this->globalVars[static::PREFIX.'wgNamespacesWithSemanticLinks'] = array_replace(
			$smwNamespacesSettings,
			$this->globalVars[static::PREFIX.'wgNamespacesWithSemanticLinks']
		);
	}
	...
	protected function getNamespacesByLanguageCode( $languageCode ) {
		$GLOBALS[static::PREFIX.'wgContLang'] = $this->extraneousLanguage->fetchByLanguageCode( $languageCode );
		return $GLOBALS[static::PREFIX.'wgContLang']->getNamespaces();
	}
